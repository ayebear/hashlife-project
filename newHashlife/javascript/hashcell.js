...
function HashCell(board, id, children){ //Class LifeNode
...
/* Private */
	...
	function getSavedCenter(step1){
		...
		for (var i = 0; i < 9; i++)
		{
			result[i] = self.subquad(i).nextCenter(step1);
		}
	...
	function center(){
		if (typeof(self._cache[0]) != "undefined" && self._cache.length > 0){
			return self._cache[0];
		}
			
		var nw = self._children[0];
		var ne = self._children[1];
		var sw = self._children[2];
		var se = self._children[3];
		
		var result = self._board.getnode(nw._children[3], ne._children[2], sw._children[1], se._children[0]);
		self._cache[0] = result;
	...
	this.subquad = function(i){
		...
		if (i == 0){ return nw;}
		if (i == 1){ return self._board.getnode(nw._children[1], ne._children[0],nw._children[3], ne._children[2]);}
		if (i == 2){ return ne;}
		if (i == 3){ return self._board.getnode(nw._children[2], nw._children[3],sw._children[0], sw._children[1]);}
		if (i == 4){ return center();}
		if (i == 5){ return self._board.getnode(ne._children[2], ne._children[3],se._children[0], se._children[1]);}
		if (i == 6){ return sw;}
		if (i == 7){ return self._board.getnode(sw._children[1], se._children[0],sw._children[3], se._children[2]);}
		if (i == 8){ return se;}
	};
