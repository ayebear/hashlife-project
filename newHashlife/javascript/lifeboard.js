...
function LifeBoard(){
	...
	var E = new HashCell(that, 0, null); //we are passing the board, id and children
	var X = new HashCell(that, 1, null);
	...
	for (var i = 0; i < 16; i++)
	{
		...
		_memo[tup] = new HashCell(this, i + 2, objtup); 
	}
	...
	this.getnode = function(nw, ne, sw, se){
		...
		if (typeof(_memo[tup]) != "undefined"){
			...
			result = new HashCell(this, _nextid, [nw, ne, sw, se]);
			_nextid = _nextid + 1;
			_memo[tup] = result;
		}
